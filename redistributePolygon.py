...
#
def calculate_field_proportion_based_on_number_of_lots(field_to_calculate, total_properties_field, local_number_of_properties_field):
	"""
...
## Script arguments
#
redistribution_parameter = arcpy.GetParameterAsText(0)
if redistribution_parameter == '#' or not redistribution_parameter:
    #raise ValueError('You must provide a redistribution_parameter')
    redistribution_parameter = r'SouthernSuburbs_PS_Catchments' # provide a default value if unspecified
...
redistribution_layers = [redistribution_parameter, ]
...
if arcpy.Exists(workspace + "data_layer"):
	print("The scratch data_layer will be delted and replaced: %s" % workspace + "data_layer")
	arcpy.Delete_management(workspace + "data_layer")
#arcpy.CopyFeatures_management(data_layer, workspace + "data_layer")
arcpy.Select_analysis (data_layer, workspace + "data_layer")
data_layer = workspace + "data_layer"
print("new data layer location: %s" % data_layer)
#
distribution_method = arcpy.GetParameterAsText(2)
if distribution_method == '#' or not distribution_method:
    distribution_method = 2 # 1 - by area, 2 - by number of properties.
...
field_list = ["POP_2011", "Total_2011", "POP_2016", "Total_2016"]
...
add_property_count_to_layer_x_with_name_x(data_layer, "GMZ_counted_prop") # TODO: change the scrips to that GMZ_num_prop is calculated by summing the loc_num_prop values of the intersecting polygons.
...
for item in redistribution_layers:
	...
	## Add total and local GMZ fields:
	local_number_of_properties_field = "loc_num_prop"
	total_properties_field = "tot_num_prop"
	...
	#renameFieldMap(fm_total_properties_rejoined, total_properties_field)
	type_name = fm_total_properties_rejoined.outputField
	type_name.name = total_properties_field
	#type_name.type = "String"
	#type_name.length = 100
	fm_total_properties_rejoined.outputField = type_name
	#fm_total_properties_rejoined.mergeRule = "Max"
	fm_total_properties_rejoined.mergeRule = "Sum"
	#fm_total_properties_rejoined.joinDelimiter = ", "
	fieldmappings_data.addFieldMap(fm_total_properties_rejoined)
	## data_layer ID: # commented out because this may be able t be done with object ID and FID, meaning it could be applied to any layer.
	#fm_GMZ_number = arcpy.FieldMap()
	#fm_GMZ_number.addInputField(data_layer, "GMZ")
	#fieldmappings_data.addFieldMap(fm_GMZ_number)
	## data_layer ID: 
	#fm_object_id = arcpy.FieldMap()
	#fm_object_id.addInputField(data_layer, "OBJECTID")
	#fm_object_id.outputField.name = "GMZ_OBJECTID"
	#fieldmappings_data.addFieldMap(fm_object_id)
	print("    Creating %s layer" % data_layer_rejoined)
	if arcpy.Exists(data_layer_rejoined):
		print("    The following file already exists and wil now be deleted: %s" % data_layer_rejoined)
		arcpy.Delete_management(data_layer_rejoined)
	...
	#arcpy.SpatialJoin_analysis (data_layer, intersecting_polygns, data_layer_rejoined, "JOIN_ONE_TO_ONE", "KEEP_ALL", fieldmappings_data, "INTERSECT")
	...
	print("    %s includes %s field" % (data_layer_rejoined, total_properties_field))
	arcpy.JoinField_management (data_layer, "OBJECTID", data_layer_rejoined,  "TARGET_FID", total_properties_field)
	print("    %s field joined to %s" % (total_properties_field, data_layer_rejoined))
	...
	for GM_field in field_list:
		#TODO: delete if the method works
		...
		if field_exists_in_feature_class(GM_field, intersecting_polygons):
			if distribution_method == 1:
				...
				calculate_field_proportion_based_on_number_of_lots(GM_field, total_properties_field, local_number_of_properties_field) #TODO: check that these parameters are correct
